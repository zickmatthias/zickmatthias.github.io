...
var portfolioItemClick = function() {
	...
	$('body').on('click', '.js-close-portfolio', function() {
		...
		TweenMax.set('.portfolio-wrapper', { visibility: 'visible', height: 'auto', display: 'block' });
		TweenMax.to('.portfolio-single-inner', 1, { 
			marginTop: '50px', 
			opacity: 0,  
			display: 'none', 
			onComplete() {
				// Portfolio-Detail entfernen:
				$('#portfolio-single-holder').empty();
				// Portfolio-Grid wieder anzeigen:
	...
	$.ajax({
		...
		success: function(html) {
			...
			TweenMax.to('.portfolio-wrapper', 1, { 
				...
				visibility: 'hidden', 
				onComplete() {
					TweenMax.set('.portfolio-wrapper', { height: 0, display: 'none' }); // display: 'none' ergänzt!
				} 
			});
			...
			setTimeout(function() {
				owlSingleSlider(id); // ID als Parameter übergeben
				$('html, body').animate({
					scrollTop: $('#portfolio-section').offset().top - 50
...
var owlSingleSlider = function (portfolioId) {
    if ( $( '.single-slider' ).length ) {
        var owl = $('.single-slider').owlCarousel({
            center: false,
            items: 1,
            loop: true,
            stagePadding: 0,
        ...
        // Zum entsprechenden Slide springen basierend auf Portfolio-ID
        if (portfolioId && portfolioId > 0) {
            var slideIndex = portfolioId - 1; // ID zu Index konvertieren (0-basiert)
            owl.trigger('to.owl.carousel', [slideIndex, 0]); // Zum entsprechenden Slide springen
        }
    }
}
